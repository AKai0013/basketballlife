...
export const SHARED_SEASON_EVENTS={
 ...
 same_team_possession:{title:"同隊關鍵回合",detail:"你們效力同一支球隊。發動、終結與防守收尾必須由不同真人接起來；責任重疊就會留下沒人處理的工作。",kind:"cooperative"},
 injury_rotation:{title:"傷病後的真人輪替",detail:"同隊真人的傷病正在改變輪替。傷者與健康隊友會收到不同任務，恢復、代扛與角色安排必須一起成立。",kind:"cooperative"},
 national_unit:{title:"國家培訓隊共同回合",detail:"具備資格的真人同時進入培訓窗口。這次只決定搭配順位、健康與評價，不會憑空增加正式國家隊出賽。",kind:"cooperative"},
 cross_league_window:{title:"跨聯盟季中交流",detail:"你們分處不同聯盟，只在合理的停賽窗口交換影片與訓練資訊。各自原球隊賽程與合約完全保留。",kind:"cooperative"}
};
...
export function midseasonEventFor(rows,year){if(rows.length<2)return null;const sameTeam=new Set(rows.map(row=>`${row.league}|${row.team_name}`)).size===1,sameLeague=new Set(rows.map(row=>row.league)).size===1,allHbl=rows.every(row=>row.league==="HBL"),allCollege=rows.every(row=>!pro(row)&&row.league!=="HBL"),allPro=rows.every(pro),injured=rows.some(row=>+row.health<=55||flags(row).injured),eligible=rows.every(row=>flags(row).nationalEligible!==false);let type="";
 ...
 const source=SHARED_SEASON_EVENTS[type],options=source.kind==="competitive"?competitiveOptions():cooperativeOptions();
 return{id:`shared-${year}-midseason-${type}`,type,checkpoint:"midseason",title:source.title,detail:source.detail,kind:source.kind,options};
}
