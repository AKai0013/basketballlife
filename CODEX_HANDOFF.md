...
- 本次補強：一般／國家隊關鍵戰在選擇前顯示與實際判定公式一致的預估成功率、表現值範圍與國家隊預估獎勵點；退休頁名次改由 API 回傳完整全球榜的 `global_rank`，不再用前 50 名加本人資料推算。
...
- 下一步：合併本次關鍵戰預估與全球名次修正；之後觀察正式版手機快取更新與退休名次是否恢復正常，不加入存檔自動修復。
