...
function renderCollegeDraftResult(a,results,{recordStory=true}={}){
 ...
 const max=collegeMaxYears(),focus=collegeDraftDevelopmentFocus(a),canReturn=p.grade<max;
 const returnCard=canReturn?`<div class="offerCard"><b>🎓 回到 ${p.team}</b><div class="mut">你仍保有目前的比賽履歷。以大${p.grade+1}身分回校後，下一季重點是${focus}。</div></div>`:`<div class="offerCard"><b>投入畢業公開測試</b><div class="mut">你仍可用畢業球員身分參加台灣職籃與 SBL 公開測試，爭取最後的職業入口。</div></div>`;
 const continuation=`<button class="choice" onclick="${canReturn?"stayCollege()":"openTryout()"}"><b>${canReturn?(offers.length?"放棄本屆報價，回大學繼續打":"回大學繼續打"):"參加畢業公開測試"}</b><small>${canReturn?`升上大${p.grade+1}，保留目前履歷並正常進入下一季。`:"以畢業球員身分繼續爭取台灣職籃與 SBL 的職業入口。"}</small></button>`;
 const recovery=offers.length?"":`<section class="v9DraftRecoveryAction"><small>NEXT STEP</small><b>落選不是結束</b><p>這次沒有收到邀請，但共享生涯仍會正常繼續。</p>${continuation}</section>`;
 ...
 special.innerHTML=`<div class="v9DraftResultShell">${recovery}${scouting}${receipt}<section class="v9DraftOfferSection"><div class="v9DraftSectionHead"><small>下一步</small><b>${offers.length?"比較選秀／新人結果、合約與保障條件":"落選不會結束生涯，選擇下一條路"}</b></div><div class="offerGrid">${offers.map(proOfferCard).join("")}${returnCard}</div></section></div>`;
 choices.innerHTML=continuation;
}
