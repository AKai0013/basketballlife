...
function v811AbilityPanelHTML(profile){
 ...
 return `<div class="derivedAbilityPanel"><div class="derivedAbilityHead"><div><b>場上特性分析</b><small>根據八項能力、位置與近期表現推導，不另外消耗能力點。</small></div><strong>${profile.role.label}</strong></div>${row("投射",[{label:"三分",value:profile.shooting.threePoint},{label:"接球投籃",value:profile.shooting.catchShoot},{label:"持球投籃",value:profile.shooting.pullUp},{label:"中距離",value:profile.shooting.midrange}])}${row("終結",[{label:"切入",value:profile.finishing.drive},{label:"對抗",value:profile.finishing.contact},{label:"低位",value:profile.finishing.post},{label:"籃下手感",value:profile.finishing.touch}])}${row("防守",[{label:"外圍",value:profile.defense.perimeter},{label:"內線",value:profile.defense.interior},{label:"協防",value:profile.defense.help},{label:"護框",value:profile.defense.rim}])}<div class="derivedAbilityNotes"><span>已形成特性：${profile.traits.length?profile.traits.slice(0,2).map(item=>item.label).join("、"):"尚未形成明確專長"}</span><span>成長方向：${profile.growthDirection.label}</span></div></div>`;
}
