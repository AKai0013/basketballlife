...
function v8AvatarImage(avatarSeed,age){
 const src=v8AvatarSrc(avatarSeed,age);if(v8AvatarImageCache.has(src))return v8AvatarImageCache.get(src);
...
function drawPlayerAvatarCanvas(c,x,y,size,avatarSeed,pos="PG",age=16){
 const record=v8AvatarImage(avatarSeed,age);c.save();c.beginPath();c.rect(x,y,size,size);c.clip();c.fillStyle="#0b151c";c.fillRect(x,y,size,size);if(record.ready&&record.img.naturalWidth)c.drawImage(record.img,x,y,size,size);c.restore();
}
function preloadPlayerAvatarSprites(){return p?v8AvatarImage(p.avatarSeed,p.age).promise:Promise.resolve()}
