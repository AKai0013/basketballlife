...
function retirementSharedWorldHTML(){
 const shared=p?.onlineSharedWorld,history=shared?.history||[],end=shared?.epilogue;if(!shared||(!history.length&&!end))return "";
 ...
 const epilogue=end?`<div class="sharedWorldFinale"><small>共享世界共同終章・${escapeFeedText(end.years||"")}</small><h3>${escapeFeedText(end.title||"共同生涯完成")}</h3><p>${escapeFeedText(end.detail||"")}</p><div class="retirementEvidence"><span>共同事件 ${Number(end.events)||0}</span><span>合作分數 ${Number(end.cooperation)>=0?"+":""}${Number(end.cooperation)||0}</span><span>共同成就 ${Number(end.jointAchievements)||0}</span><span>真人對決 ${Number(end.rivalries)||0}</span></div></div>`:"";
 return `<section class="eraPeople sharedRetirementHistory"><div class="echoIntro"><span>共同生涯</span><h2>${end?"所有真人球員都已完成這條時間線。":"你的生涯，也改變了真人隊友的時間線。"}</h2><p>世界 ${escapeFeedText(shared.code)}｜共同事件 ${history.length} 次｜合作分數 ${cooperation>=0?"+":""}${cooperation}${people.size?`｜${[...people].map(escapeFeedText).join("・")}`:""}</p></div>${epilogue}<div class="eraPeopleGrid">${history.slice(-6).reverse().map(row=>`<article><small>${row.year}｜${escapeFeedText(row.label||"共同選擇")}</small><b>${escapeFeedText(row.title||"共同事件")}</b><span>${escapeFeedText((row.causedBy||[])[0]||row.detail||"結果已寫入共同時間線")}</span></article>`).join("")}</div></section>`;
}
