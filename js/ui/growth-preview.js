(() => {
  ...
  function v9CareerRecordMarkup(player) {
    ...
    const peopleHTML = people.length ? `<section class="v9StorySection"><div class="v9StorySectionHead"><small>CAREER CAST</small><b>這段生涯裡的人</b></div><div class="v9CareerPeople">${people.map((person) => `<article><span>${safeText(person.label)}</span><b>${safeText(person.name)}</b><small>${safeText(person.note)}</small></article>`).join("")}</div></section>` : `<section class="v9StorySection"><div class="v9StorySectionHead"><small>CAREER CAST</small><b>這段生涯裡的人</b></div><div class="v9DrawerEmpty">人物會在正式事件中登場；遇見以前不提前顯示姓名。</div></section>`;
