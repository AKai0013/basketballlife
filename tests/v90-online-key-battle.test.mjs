...
test("season boundary waits for every human and opens one shared intersection",async()=>{const {database,DB}=await setup(),{code}=await createWorld(DB);let state=await call(DB,users[0],`rooms/${code}/sync`,"POST",snapshot(true));assert.equal(state.status,200);assert.equal(state.body.advance,false);assert.equal(state.body.players.filter(row=>row.ready).length,1);state=await call(DB,users[1],`rooms/${code}/sync`,"POST",snapshot(true));assert.equal(state.body.advance,false);assert.ok(state.body.players.every(row=>row.ready));assert.equal(state.body.intersection.title,"HBL 全國賽交會");database.close()});

test("both intersection choices create a shared result and each client advances once",async()=>{const {database,DB}=await setup(),{code}=await createWorld(DB);await call(DB,users[0],`rooms/${code}/sync`,"POST",snapshot(true));await call(DB,users[1],`rooms/${code}/sync`,"POST",snapshot(true));let state=await call(DB,users[0],`rooms/${code}/intersection`,"POST",{year:2026,choice:"duel"});assert.equal(state.body.advance,false);assert.equal(state.body.submitted,true);state=await call(DB,users[1],`rooms/${code}/intersection`,"POST",{year:2026,choice:"connect"});assert.equal(state.body.advance,true);assert.equal(state.body.result.effect.health,2);state=await call(DB,users[0],`rooms/${code}/sync`,"POST",snapshot(true));assert.equal(state.body.advance,true);assert.equal(state.body.result.effect.rep,4);assert.equal(database.prepare("SELECT timeline_year FROM online_shared_worlds WHERE room_code=?").get(code).timeline_year,2027);database.close()});
